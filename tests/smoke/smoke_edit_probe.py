"""
Smoke test to probe chat.edited_message WA-JS listener and show its raw output.
"""
...
async def main():
    ...
    print(f"[*] Registering listener: {WaJSEvents.EDIT}")
    await wapi.bridge.register_listener(WaJSEvents.EDIT)

    print("[*] Polling for EDIT events... Edit a sent message in WhatsApp!")
    try:
        while True:
            events = await wapi.bridge.drain_queue_for(EventName.EDIT_EVENT)
            for e in events:
                print("\n--- New EDIT Event ---")
                print(e)
